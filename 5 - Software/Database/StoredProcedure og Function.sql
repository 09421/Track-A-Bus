...
BEGIN
	...
	drop table if exists possibleRoutes;
	...
	join BusStop on BusRoute_BusStop.ID = BusStop.ID
	where BusStop.RouteNumber = routeNumber and BusStop.StopName = stopName;
	...
	call GetClosestBusAscProc(StopId,ClosestEndPointIdAsc, ClosestBusDistanceAsc, ClosestBusIdAsc );
	Call GetClosestBusDescProc(StopId, ClosestEndPointIdDesc, ClosestBusDistanceDesc, ClosestBusIdDesc ); 
...
begin 
	...
	set closestBusId = closestsBusID;
...
begin
	...
	insert into ChosenRouteAsc (bus_lat,bus_lon)
	select RoutePoint.Latitude,  RoutePoint.Longitude from RoutePoint
...
begin
	...
	insert into ChosenRouteDesc  (bus_lat,bus_lon)
	select RoutePoint.Latitude,  RoutePoint.Longitude from RoutePoint
	inner join BusRoute_RoutePoint on BusRoute_RoutePoint.fk_RoutePoint = RoutePoint.ID
	inner join Bus on Bus.fk_BusRoute = BusRoute_RoutePoint.fk_BusRoute
	where Bus.ID = busID
